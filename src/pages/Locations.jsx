...
const airportIcon = new L.DivIcon({
  html: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36" width="36" height="36">
    <circle cx="18" cy="18" r="16" fill="#2563EB" stroke="#F3F4F6" stroke-width="2"/>
    <path d="M18 8l-2 7h-6l-1.5 3 7 2v5l-2 2h4l1.5-2 1.5 2h4l-2-2v-5l7-2L28 15h-6l-2-7h-2z" fill="#F3F4F6"/>
  </svg>`,
  className: '',
  iconSize: [36, 36],
  iconAnchor: [18, 18],
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {filters.map(f => (
              <button
                ...
                className={`px-5 py-2.5 text-sm font-bold uppercase tracking-[0.1em] transition-all duration-300 ${
                  activeFilter === f.key
                    ? 'bg-lime text-white'
                    : 'border border-black/10 text-ash hover:text-chalk hover:border-black/20'
                }`}
          ...
          <div className="flex flex-col lg:flex-row gap-px">
            ...
            <div className="w-full lg:w-3/5">
              <div className="border border-black/10 overflow-hidden relative" style={{ height: '600px' }}>
                <MapContainer
                  center={[-26.5, 119]}
                  zoom={5}
                ...
                {/* Legend */}
                <div className="absolute bottom-4 left-4 z-[1000] bg-white/90 border border-black/10 px-4 py-3 flex items-center gap-4">
            ...
            <div className="w-full lg:w-2/5">
              <div className="bg-charcoal-light border border-black/10 overflow-hidden" style={{ height: '600px' }}>
                <div className="px-5 py-4 border-b border-black/10 flex items-center justify-between">
                  <h3 className="text-chalk font-bold uppercase tracking-[0.1em] text-sm">
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-black/5">
                      {filtered.map((point) => (
                        <tr
                          key={point.id}
                          className={`cursor-pointer transition-colors duration-200 hover:bg-black/5 ${selectedPoint?.id === point.id ? 'bg-lime/10 border-l-2 border-l-lime' : ''}`}
